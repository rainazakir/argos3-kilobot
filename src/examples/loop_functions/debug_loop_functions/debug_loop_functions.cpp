...
/****************
https://github.com/DiODeProject/AdaptationStudy/blob/master/ARGoS_simulation/loop_functions/ark.cpp
to get variables from experiment file
************************/
/****************************************/
...
/****************************************/
/****************************************/
void CDebugLoopFunctions::PreStep(){
...
void CDebugLoopFunctions::PostStep() {
   ...
   for(size_t i = 0; i < m_tKBs.size(); ++i) {
      ...
      /* Print current state internal robot state */
       ...
       if(m_tKBs[i].second->currentopinion == 1){
           op1 +=1;
       }
              
       if(m_tKBs[i].second->currentopinion == 2){
           op2 +=1;
       }
       
       
      // m_tKBs[i].second->currentopinion 
/*
      LOG << m_tKBs[i].first->GetId() << ": "                       << std::endl
          << "\trx_message[KILOBOT_MAX_RX]: " << ptState->tx_state          << std::endl
          ...
          << "\tOpinion: "           << m_tKBs[i].second->currentopinion << std::endl;
       
    m_cOutput << m_tKBs[i].first->GetId() << ": "                       << std::endl
    ...
    */
        ...
        m_cOutput << GetSpace().GetSimulationClock() << "    " << op1 << " " << op2 <<  " "   << std::endl;
